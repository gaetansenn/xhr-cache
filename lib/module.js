...
function handleRefresh (store, id, maxAge, name, conf) {
  ...
  if (maxAge) {
    resource.refresh.intervalId = setInterval(store, maxAge)
    ...
    const stop = function () {
      ...
      if (refreshResourceIndex !== -1) {
        ...
        /* eslint-disable-next-line */
        console.info(`${libPrefix} Stop auto refresh for '${name} with id '${id}'`)
      }
    }
...
function registerResource ({ store, get, id, ctx, name, maxAge }) {
  const resource = resources.find(resource => resource.id === id)
  ...
  if (maxAge) { _resource.refresh = { maxAge } }
